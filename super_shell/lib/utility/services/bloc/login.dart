...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  Future<void> loginWithApple(LoginAppleEvent event, Emitter<LoginState> emit) {
    ...
    return svc<LoginService>().signInWithApple().then((value) => loginSuccess(emit)).catchError((e, es) => loginError(emit, e, es));
  }

  Future<void> loginWithGoogle(LoginGoogleEvent event, Emitter<LoginState> emit) {
    emit(LoginState.loading);
    return svc<LoginService>().signInWithGoogle().then((value) => loginSuccess(emit)).catchError((e, es) => loginError(emit, e, es));
  }
  ...
  Future<void> loginSuccess(Emitter<LoginState> emit) {
    return svc<UserService>().bind(svc<UserService>().uid()).then((value) => emit(LoginState.success)).then((value) => emit(LoginState.normal));
  }
}
